...
function extractDynamicConfig(url) {
    ...
    const qPort = parsed.searchParams.get("port");
...
// ============================================
function generateAllConfigs(host, activeUUID, activeTrojanPass, activeWSPath, activeProxy, customPort = null) {
    const rawHost = host.split(":")[0];
    const cleanPath = (activeWSPath || DEFAULT_WS_PATH).replace(/^\/+/, "");
    const titleHost = rawHost.replace(/[^a-zA-Z0-9.-]/g, "");
    
    // Trojan password defaults to UUID if not specified separately
    const trojanKey = activeTrojanPass || activeUUID;

    // 1. VLESS Configurations (including Port 8080)
    const vlessTls = `vless://${activeUUID}@${rawHost}:443?encryption=none&security=tls&sni=${rawHost}&type=ws&host=${rawHost}&path=%2F${encodeURIComponent(cleanPath)}%3Fed%3D2048#Galaxy-VLESS-TLS%20(${titleHost})`;
    const vlessGrpc = `vless://${activeUUID}@${rawHost}:443?encryption=none&security=tls&sni=${rawHost}&type=grpc&serviceName=${encodeURIComponent(cleanPath)}#Galaxy-VLESS-gRPC%20(${titleHost})`;
    const vlessHttp = `vless://${activeUUID}@${rawHost}:80?encryption=none&security=none&type=ws&host=${rawHost}&path=%2F${encodeURIComponent(cleanPath)}%3Fed%3D2048#Galaxy-VLESS-HTTP%20(${titleHost})`;
    const vless8080 = `vless://${activeUUID}@${rawHost}:8080?encryption=none&security=none&type=ws&host=${rawHost}&path=%2F${encodeURIComponent(cleanPath)}%3Fed%3D2048#Galaxy-VLESS-8080%20(${titleHost})`;
    
    let vlessProxy = "";
    let vlessProxy8080 = "";
    if (activeProxy) {
        const rawProxy = activeProxy.split(":")[0];
        vlessProxy = `vless://${activeUUID}@${rawProxy}:443?encryption=none&security=tls&sni=${rawHost}&type=ws&host=${rawHost}&path=%2F${encodeURIComponent(cleanPath)}%3Fed%3D2048#Galaxy-VLESS-ProxyIP%20(${rawProxy})`;
        vlessProxy8080 = `vless://${activeUUID}@${rawProxy}:8080?encryption=none&security=none&type=ws&host=${rawHost}&path=%2F${encodeURIComponent(cleanPath)}%3Fed%3D2048#Galaxy-VLESS-ProxyIP-8080%20(${rawProxy})`;
    }

    // 2. Trojan Configurations (Dual Compatibility with same UUID or Password)
    const trojanTls = `trojan://${encodeURIComponent(trojanKey)}@${rawHost}:443?security=tls&sni=${rawHost}&type=ws&host=${rawHost}&path=%2F${encodeURIComponent(cleanPath)}%3Fed%3D2048#Galaxy-Trojan-TLS%20(${titleHost})`;
    const trojanGrpc = `trojan://${encodeURIComponent(trojanKey)}@${rawHost}:443?security=tls&sni=${rawHost}&type=grpc&serviceName=${encodeURIComponent(cleanPath)}#Galaxy-Trojan-gRPC%20(${titleHost})`;
    
    let trojanProxy = "";
    if (activeProxy) {
        const rawProxy = activeProxy.split(":")[0];
        trojanProxy = `trojan://${encodeURIComponent(trojanKey)}@${rawProxy}:443?security=tls&sni=${rawHost}&type=ws&host=${rawHost}&path=%2F${encodeURIComponent(cleanPath)}%3Fed%3D2048#Galaxy-Trojan-ProxyIP%20(${rawProxy})`;
    }
    ...
    if (vlessProxy8080) configs.push(vlessProxy8080);
    if (trojanProxy) configs.push(trojanProxy);

    // If specific port requested like ?port=8080, prioritize/filter to that port
    if (customPort === "8080" || customPort === 8080) {
        configs = [vless8080];
        if (vlessProxy8080) configs.push(vlessProxy8080);
    } else if (customPort && customPort !== "443" && customPort !== "80") {
        const customVless = `vless://${activeUUID}@${rawHost}:${customPort}?encryption=none&security=none&type=ws&host=${rawHost}&path=%2F${encodeURIComponent(cleanPath)}%3Fed%3D2048#Galaxy-VLESS-${customPort}%20(${titleHost})`;
        configs.unshift(customVless);
    }
...
var worker_default = {
    async fetch(request, env = {}, ctx) {
        ...
        if (dynConfig.isSubRequest || url.pathname === "/sub" || url.searchParams.has("sub")) {
            const subData = generateAllConfigs(host, runtimeUUID, runtimeTrojanPass, runtimeWSPath, runtimeProxyIP, dynConfig.port);
...
<body>
  ...
  <div class="main-container">
    ...
    <div class="config-panel">
      ...
      <div class="node-list">
        ...
        <div class="node-card">
          <div class="node-info">
            <div class="node-name">🌐 VLESS + Direct HTTP (Port 8080)</div>
            <div class="node-desc" id="http8080Desc">Cloudflare Alternate WS Port 8080</div>
          </div>
          <button class="copy-btn" onclick="copyNode('vless8080')">Copy Link</button>
        </div>

        <div class="node-card" id="proxyCard8080" style="display: none;">
          <div class="node-info">
            <div class="node-name">🛡️ VLESS + ProxyIP (Port 8080)</div>
            <div class="node-desc" id="proxy8080Desc">ProxyIP Non-TLS WS Port 8080</div>
          </div>
          <button class="copy-btn" onclick="copyNode('vlessProxy8080')">Copy Link</button>
        </div>

        <div class="node-card" id="proxyCard" style="display: none;">
          <div class="node-info">
            <div class="node-name">🛡️ VLESS + ProxyIP (Port 443)</div>
            <div class="node-desc" id="proxyDesc">ProxyIP TLS WS Port 443</div>
          </div>
  ...
  <script>
    ...
    const currentHost = (window.location.hostname || "${host}").split(":")[0];
    let generatedLinks = {};
    ...
    function updateConfigs() {
      ...
      const vless8080 = "vless://" + uuid + "@" + currentHost + ":8080?encryption=none&security=none&type=ws&host=" + currentHost + "&path=%2F" + encodeURIComponent(path) + "%3Fed%3D2048#Galaxy-VLESS-8080%20(" + cleanHost + ")";

      let vlessProxy = "";
      let vlessProxy8080 = "";
      if (proxy) {
        const rawProxy = proxy.split(":")[0];
        vlessProxy = "vless://" + uuid + "@" + rawProxy + ":443?encryption=none&security=tls&sni=" + currentHost + "&type=ws&host=" + currentHost + "&path=%2F" + encodeURIComponent(path) + "%3Fed%3D2048#Galaxy-VLESS-ProxyIP%20(" + rawProxy + ")";
        vlessProxy8080 = "vless://" + uuid + "@" + rawProxy + ":8080?encryption=none&security=none&type=ws&host=" + currentHost + "&path=%2F" + encodeURIComponent(path) + "%3Fed%3D2048#Galaxy-VLESS-ProxyIP-8080%20(" + rawProxy + ")";
        const pCard = document.getElementById('proxyCard');
        const pCard8080 = document.getElementById('proxyCard8080');
        if (pCard) pCard.style.display = 'flex';
        if (pCard8080) pCard8080.style.display = 'flex';
      } else {
        const pCard = document.getElementById('proxyCard');
        const pCard8080 = document.getElementById('proxyCard8080');
        if (pCard) pCard.style.display = 'none';
        if (pCard8080) pCard8080.style.display = 'none';
      }
      ...
      if (proxy) {
        const rawProxy = proxy.split(":")[0];
        trojanProxy = "trojan://" + encodeURIComponent(trojanKey) + "@" + rawProxy + ":443?security=tls&sni=" + currentHost + "&type=ws&host=" + currentHost + "&path=%2F" + encodeURIComponent(path) + "%3Fed%3D2048#Galaxy-Trojan-ProxyIP%20(" + rawProxy + ")";
      }

      const allLinks = [vlessTls, trojanTls, vlessGrpc, trojanGrpc, vlessHttp, vless8080];
      if (vlessProxy) allLinks.push(vlessProxy);
      if (vlessProxy8080) allLinks.push(vlessProxy8080);
      if (trojanProxy) allLinks.push(trojanProxy);
    ...
    function copyNode(type) {
      ...
      if (generatedLinks[type]) {
        navigator.clipboard.writeText(generatedLinks[type]).then(() => {
          const names = {
            vlessTls: 'VLESS TLS (Port 443)',
            vlessGrpc: 'VLESS gRPC (Port 443)',
            vlessHttp: 'VLESS HTTP (Port 80)',
            vless8080: 'VLESS HTTP (Port 8080)',
            vlessProxy: 'VLESS ProxyIP (Port 443)',
            vlessProxy8080: 'VLESS ProxyIP (Port 8080)',
            trojanTls: 'Trojan TLS (Port 443)',
            trojanGrpc: 'Trojan gRPC (Port 443)',
            trojanProxy: 'Trojan ProxyIP (Port 443)'
          };
          showToast('Copied ' + (names[type] || type.toUpperCase()) + ' link!');
        });
      }
    }
